...
@section('content')
    ...
    @if ($magang)
        <div
            class="bg-emerald-50 border border-emerald-200 p-5 rounded-3xl mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm">
            ...
            <span
                class="px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide bg-emerald-200/50 text-emerald-700 border border-emerald-200">
                {{ $magang->status }}
            </span>
        </div>
    @else
        <div
            class="bg-rose-50 border border-rose-200 p-5 rounded-2xl mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm">
            <div class="flex items-center gap-3">
            ...
            <span
                class="px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide bg-rose-200/50 text-rose-700 border border-rose-200">
                Belum Ada
            </span>
        </div>
    @endif
    ...
    <div
        class="bg-white/70 backdrop-blur-2xl rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-white/80 overflow-hidden mb-24">
        <div class="bg-gradient-to-r from-blue-600 to-cyan-500 px-6 py-5">
            ...
            <p class="text-blue-100 text-sm mt-1 font-medium">Harap isi data berikut dengan lengkap dan benar sesuai SK
                Magang Anda.</p>
        </div>
        ...
        <form method="POST" action="{{ route('magang.peserta.update') }}" class="p-6 md:p-8 fade-in-form" id="biodata-form">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="input-group">
                    <label class="block text-sm font-semibold text-gray-700 mb-2" for="instansi">
                        <i class="fas fa-university text-gray-400 mr-1.5 w-4 text-center input-icon"></i> Asal Instansi /
                        Universitas
                    </label>
                    <input type="text" id="instansi" name="instansi" value="{{ old('instansi', $magang->instansi ?? '') }}"
                ...
                <div class="input-group">
                    <label class="block text-sm font-semibold text-gray-700 mb-2" for="pembimbing">
                        <i class="fas fa-user-tie text-gray-400 mr-1.5 w-4 text-center input-icon"></i> Nama Pembimbing
                        Lapangan
                    </label>
                    <input type="text" id="pembimbing" name="pembimbing"
                        value="{{ old('pembimbing', $magang->pembimbing ?? '') }}" placeholder="Contoh: La Ode Haerul Saleh"
                        class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all outline-none text-gray-800">
                </div>

                <div class="input-group">
                    <label class="block text-sm font-semibold text-gray-700 mb-2" for="tanggal_mulai">
                        <i class="fas fa-calendar-alt text-gray-400 mr-1.5 w-4 text-center input-icon"></i> Tanggal Mulai
                        Magang
                    </label>
                ...
                <div class="input-group">
                    <label class="block text-sm font-semibold text-gray-700 mb-2" for="tanggal_selesai">
                        <i class="fas fa-calendar-check text-gray-400 mr-1.5 w-4 text-center input-icon"></i> Tanggal
                        Selesai Magang
                    </label>
            ...
            <div class="mb-8 input-group">
                <label class="block text-sm font-semibold text-gray-700 mb-2" for="catatan">
                    <i class="fas fa-sticky-note text-gray-400 mr-1.5 w-4 text-center input-icon"></i> Catatan Tambahan
                    (Opsional)
                </label>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Spinner dan confetti saat submit
